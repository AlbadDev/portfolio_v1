    ...
    return (
            <section>
                <section    id={work.id} className={style.section}>
                    <header>
                        ...
                        <div className={style.left}>
                            ...
                            <Link href='/projectInfos/[id]' as={`/projectInfos/${work.id}`}>
                                <h2 style={{width:"100%"}}><Reveal>{work.title}</Reveal></h2>
                            </Link>
                        </div>
                    ...
                    <Reveal><p>{work.description}</p></Reveal>
                    
                    <footer className={style.footer}>
                        
                        {work.material.map(tool => <ul key={tool.toString()}><li ><Reveal>{tool}</Reveal></li></ul>)}
                    </footer>
                </section>
            </section>
    )
